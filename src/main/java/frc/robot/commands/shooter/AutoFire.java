...
public class AutoFire extends CommandBase {
    ...
    public AutoFire(Drivetrain drivetrain, Shooter shooter, Magazine magazine, Intake intake, Limelight limelight, Turret turret, Hood hood, boolean cont) {
    ...
    public void execute() {
        ...
        double setRPM = (((drivetrain.getLeftVelocity() + drivetrain.getRightVelocity()) / 2) > 0.2) ? movingRPM : ((raised) ? 2000 : limelight.formulaRpm());
        shooter.setVelocity(setRPM);

        boolean seen = (raised) ? true : limelight.hasTarget();
        boolean aimed = (raised) ? true : (limelight.getIsAimed() && seen);
        ...
        SmartDashboard.putBoolean("seen", seen);
        SmartDashboard.putBoolean("aimed", aimed);
        SmartDashboard.putBoolean("rasied", raised);
