...
public class IntakeCam {
  ...
  private static IntakeCam instance = null;

  public static synchronized IntakeCam getInstance() {
    if (instance == null) instance = new IntakeCam();
  ...
  private IntakeCam() {
    grip = NetworkTableInstance.getDefault().getTable("Balls");
  ...
  public double getCloseX(double cameraAngle) {
    ...
    if (getXs().length == 0) return 0.0;
      ...
      int index = 0;
      double max = 0.0;
      // Iterate through the Y values and whenever a Y value is the highest seen so far, save the
      // index.
